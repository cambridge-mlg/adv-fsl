...
def generate(path, dataset, testset, device, net, delta=0.2, max_iter_uni=np.inf, xi=10, p=np.inf, num_classes=10,
    ...
    while fooling_rate < 1-delta and iter < max_iter_uni:
        ...
        for k in order:
            ...
            per_img = Image.fromarray(cur_img+v.astype(np.uint8))
