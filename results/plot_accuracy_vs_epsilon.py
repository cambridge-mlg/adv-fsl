...
def plot_curves(file_list, labels, colors, line_styles, output_file, legend_location):
    fig = plt.figure()
    for file, label, color, line_style in zip(file_list, labels, colors, line_styles):
        data = np.genfromtxt(file, delimiter=',', names=['Epsilon', 'Accuracy', "Error"])
        plt.plot(data['Epsilon'], data['Accuracy'], color=color, label=label, linestyle=line_style)
    ...
    plt.legend(loc=legend_location, fontsize='x-large')
...
def main():
    ...
    plot_curves(file_list=file_list, labels=labels, colors=colors, line_styles=line_styles, output_file='./results/plots/accuracy_vs_epsilon_adv_support.pdf',
    ...
    plot_curves(file_list=file_list, labels=labels, colors=colors, line_styles=line_styles, output_file='./results/plots/accuracy_vs_epsilon_adv_query.pdf',
                legend_location='upper right')
